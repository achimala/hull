...
enum ConvexQueries {
    static func statusLogsGet<T: Decodable>(
        client: MobileConvexClient,
        args: StatusLogsGetArgs
    ) -> AsyncThrowingStream<T, Error> {
        do {
            return convexSubscribe(client: client, path: "statusLogs:get", args: try args.convexArgs(), yielding: T.self)
        } catch {
    ...
    static func statusLogsGet<T: Decodable>(
        ...
        let args = StatusLogsGetArgs(
            id: id,
        )
        return statusLogsGet(client: client, args: args)
    }

}

enum ConvexMutations {
    static func statusLogsPut<T: Decodable>(
        client: MobileConvexClient,
        args: StatusLogsPutArgs
    ) async throws -> T {
        try await convexCall(client: client, path: "statusLogs:put", args: try args.convexArgs(), remoteCall: client.mutation)
    }
    ...
    static func statusLogsPut<T: Decodable>(
        ...
        let args = StatusLogsPutArgs(
            `internal`: `internal`,
            type: type,
        )
        return try await statusLogsPut(client: client, args: args)
    }
