...
enum ConvexQueries {
    static func jobsSnapshot<T: Decodable>(
        client: MobileConvexClient,
        args: JobsSnapshotArgs
    ) -> AsyncThrowingStream<T, Error> {
        do {
            return convexSubscribe(client: client, path: "jobs:snapshot", args: try args.convexArgs(), yielding: T.self)
        } catch {
    ...
    static func jobsSnapshot<T: Decodable>(
        ...
        let args = JobsSnapshotArgs(
            id: id,
        )
        return jobsSnapshot(client: client, args: args)
    }

    static func jobsStream<T: Decodable>(
        client: MobileConvexClient,
        args: JobsStreamArgs
    ) -> AsyncThrowingStream<T, Error> {
        do {
            return convexSubscribe(client: client, path: "jobs:stream", args: try args.convexArgs(), yielding: T.self)
        } catch {
            return AsyncThrowingStream { continuation in
                continuation.finish(throwing: error)
            }
    ...
    static func jobsStream<T: Decodable>(
        ...
        let args = JobsStreamArgs(
            id: id,
        )
        return jobsStream(client: client, args: args)
    }
