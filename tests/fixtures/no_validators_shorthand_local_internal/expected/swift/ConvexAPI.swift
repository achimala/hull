...
enum ConvexQueries {
    static func tasksList<T: Decodable>(
        client: MobileConvexClient,
        args: TasksListArgs
    ) -> AsyncThrowingStream<T, Error> {
        do {
            return convexSubscribe(client: client, path: "tasks:list", args: try args.convexArgs(), yielding: T.self)
        } catch {
    ...
    static func tasksList<T: Decodable>(
        client: MobileConvexClient,
        status: TasksListArgsStatus,
    ) -> AsyncThrowingStream<T, Error> {
        let args = TasksListArgs(
            status: status,
        )
        return tasksList(client: client, args: args)
    }

}

enum ConvexMutations {
    static func tasksSetStatus<T: Decodable>(
        client: MobileConvexClient,
        args: TasksSetStatusArgs
    ) async throws -> T {
        try await convexCall(client: client, path: "tasks:setStatus", args: try args.convexArgs(), remoteCall: client.mutation)
    }

    static func tasksSetStatus<T: Decodable>(
        client: MobileConvexClient,
        id: String,
        status: TasksSetStatusArgsStatus,
    ) async throws -> T {
        let args = TasksSetStatusArgs(
            id: id,
            status: status,
        )
        return try await tasksSetStatus(client: client, args: args)
    }
