...
enum ConvexQueries {
    static func eventsLatest<T: Decodable>(
        client: MobileConvexClient,
        args: EventsLatestArgs
    ) -> AsyncThrowingStream<T, Error> {
        do {
            return convexSubscribe(client: client, path: "events:latest", args: try args.convexArgs(), yielding: T.self)
        } catch {
    ...
    static func eventsLatest<T: Decodable>(
        ...
        let args = EventsLatestArgs(
            actorId: actorId,
        )
        return eventsLatest(client: client, args: args)
    }

}

enum ConvexMutations {
    static func eventsPublish<T: Decodable>(
        client: MobileConvexClient,
        args: EventsPublishArgs
    ) async throws -> T {
        try await convexCall(client: client, path: "events:publish", args: try args.convexArgs(), remoteCall: client.mutation)
    }
    ...
    static func eventsPublish<T: Decodable>(
        ...
        let args = EventsPublishArgs(
            payload: payload,
        )
        return try await eventsPublish(client: client, args: args)
    }
