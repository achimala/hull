...
enum ConvexQueries {
    static func thingsList<T: Decodable>(
        client: MobileConvexClient,
        args: ThingsListArgs
    ) -> AsyncThrowingStream<T, Error> {
        do {
            return convexSubscribe(client: client, path: "things:list", args: try args.convexArgs(), yielding: T.self)
        } catch {
    ...
    static func thingsList<T: Decodable>(
        client: MobileConvexClient,
        cursor: String? = nil,
        includeGone: Bool? = nil,
    ) -> AsyncThrowingStream<T, Error> {
        let args = ThingsListArgs(
            cursor: cursor,
            includeGone: includeGone,
        )
        return thingsList(client: client, args: args)
    }

}

enum ConvexMutations {
    static func thingsCreate<T: Decodable>(
        client: MobileConvexClient,
        args: ThingsCreateArgs
    ) async throws -> T {
        try await convexCall(client: client, path: "things:create", args: try args.convexArgs(), remoteCall: client.mutation)
    }

    static func thingsCreate<T: Decodable>(
        client: MobileConvexClient,
        item: ThingsCreateArgsItem,
    ) async throws -> T {
        let args = ThingsCreateArgs(
            item: item,
        )
        return try await thingsCreate(client: client, args: args)
    }

}

enum ConvexActions {
    static func thingsInspect<T: Decodable>(
        client: MobileConvexClient,
        args: ThingsInspectArgs
    ) async throws -> T {
        try await convexCall(client: client, path: "things:inspect", args: try args.convexArgs(), remoteCall: client.action)
    }
    ...
    static func thingsInspect<T: Decodable>(
        ...
        let args = ThingsInspectArgs(
            id: id,
        )
        return try await thingsInspect(client: client, args: args)
    }
